...
const Navbar = () => {
    return (
        <div className='absolute top-8 left-1/2 -translate-x-1/2 gap-2 sm:gap-3 flex px-2 sm:px-0'>
            <div className='rounded-lg flex gap-[30px] sm:gap-[60px] items-center justify-center bg-primary-blue w-fit sm:py-3 sm:pl-4 sm:pr-9 py-2 px-4 border-2 border-white box-border drop-shadow-outline-white'>
                ...
                <div className='flex gap-6 sm:gap-8 items-center justify-center'>
                    {navElements.map((element, index) => (
                        <Link href={element.link} key={index} className='flex gap-1 items-center justify-center w-5 sm:w-full'>
            ...
            <Link href={'/'} className='bg-discord-violate rounded-xl border border-white drop-shadow-outline-white flex items-center justify-center sm:w-16 w-14 h-fit py-3'>
