...
export default function WithdrawSelectModal({ setSelectedOption, selectedOption }: { selectedOption: Option | null, setSelectedOption: (obj: Option) => void }) {
        ...
        <div className="relative">
            ...
            >
                <div className='flex items-center gap-2'>
                    <div className='relative flex justify-start items-center translate-y-0 w-5'>
                        ...
                        <Image src={icons[selectedOption?.icon ? selectedOption?.icon : "dogecoin"]} width={21} height={21} alt='' className='rounded-full absolute translate-x-1 translate-y-1'></Image>
                        ...
                        <ul className="space-y-2 divide-y-[1px] divide-white/15">
                            {options.map(option => (
                                <li key={option.id}>
                                    ...
                                    >
                                        <div className='relative flex justify-start items-center translate-y-0 w-5'>
                                            <Image src={'/logos/signatureIcon.svg'} width={36} height={36} alt='' className='rounded-full absolute -translate-x-1 -translate-y-1 border'></Image>
                                            <Image src={icons[option?.icon ? option?.icon : "dogecoin"]} width={36} height={36} alt='' className='rounded-full absolute translate-x-1 translate-y-1'></Image>
